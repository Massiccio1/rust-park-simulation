...
fn main() {
    ...
    loop{
        ...
        println!("fibonacci di {num}= {fib2}");
        let fib=fibonacci_fast(num, true);
        println!("fibonacci di {num}= {fib}");
    }
...
fn input_type<T:FromStr>(_type:T) -> T {
    ...
    loop {
        ...
        // Trim whitespace and parse the string as an integer
...
fn fibonacci_slow(depth:i32, verbose: bool) -> i128{
    ...
    let sum = fibonacci_slow(depth-1, true) + fibonacci_slow(depth-2, true);
    if verbose{
        println!("{}", sum);
    }
    return sum;
